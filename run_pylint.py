...
try:
    ...
    print("""Error, Module pylint is required
********************************
...
DISABLED_CHECKS = [
    "missing-docstring", "no-else-return", "import-error", "no-self-use"
]
BASE_OPTIONS = "--const-naming-style=any"
PYTHON_EXTENSION = ".py"
SCORE_REGEX = (
    r"-+\s+Your code has been rated at (-?[0-9\.]+)\/10( \(previous "
    r"run: -?[0-9\.]+\/10, [-+][0-9\.]+\))?")
    ...
    def _decorate(function):
        ...
        def crash_handler(*args, **kwargs):
            ...
            # pylint: disable=broad-except
                ...
                print(
                    "Please make a private post on Piazza with the following information"
                )
...
def main(root=None, verbose=False, process_count=None, strict=False):
    ...
    files = [
        f for f in find_files(path, PYTHON_EXTENSION)
        if not f.endswith(current_script)
    ]
...
def get_options(additional_options, strict=False):
    ...
    return ["--disable={}".format(",".join(DISABLED_CHECKS)), BASE_OPTIONS
            ] + additional_options
...
def bootstrap():
    ...
    parser.add_argument(
        ...
        metavar="path",
        help=
        "the path to run pylint over (defaults to current working directory)")
    parser.add_argument(
        "--parallel",
        "-p",
        "-j",
    ...
    main(root=parsed_args.root,
         process_count=parsed_args.parallel,
         verbose=parsed_args.verbose,
         strict=parsed_args.all)
